...
#                    code to test
#   SQUASHFS_FILE:   The squashfs file name to run rootfs_size against
...
SQUASHFS_FILE=${SQUASHFS_FILE:-"squashfs-xz"}
...
docker run --cap-add=sys_admin --rm=true \
    ...
    -t ${DOCKER_IMG_NAME} \
    "${DOCKER_WORKDIR}"/${ROOTFS_SIZE_PY} --build_dir ${WORKSPACE}/ --squashfs_file ${SQUASHFS_FILE}
