...
#   USE_CACHED_DOCKER_IMG: Optional, do not build docker images; using the specified cached one
...
USE_CACHED_DOCKER_IMG="${USE_CACHED_DOCKER_IMG:-}"
...
if [ -z "${USE_CACHED_DOCKER_IMG}" ]; then
    ...
    export DOCKER_IMG_NAME=${USE_CACHED_DOCKER_IMG}
fi
