...
UNIT_TEST_SH="https://raw.githubusercontent.com/openbmc/"
UNIT_TEST_SH+="openbmc-build-scripts/scripts/unit-tests.sh"
...
# Pull down unit test execution script into workspace
echo "Retrieving unit test execution script to ${WORKSPACE}"
#wget ${UNIT_TEST_SH} ${WORKSPACE}/${UNIT_TEST_SH}
wget https://gerrit.openbmc-project.xyz/cat/1214%2C2%2Cscripts/unit-test.sh%5E0 -O ${WORKSPACE}/unit_test.sh.zip
echo "Unzipping ${WORKSPACE}/unit_test.sh.zip"
UNIT_TEST_SH="unit_test.sh"
unzip -p ${WORKSPACE}/unit_test.sh.zip > ${WORKSPACE}/${UNIT_TEST_SH}
...
docker run --cap-add=sys_admin --rm=true -e WORKSPACE=${WORKSPACE} \
    --user="${USER}" -w "${HOME}" -v "${HOME}":"${HOME}" \
