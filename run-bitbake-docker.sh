...
###############################################################################
#
# run-bitbake-docker.sh script is for running the OpenBMC builds as containers
# with the option of launching the containers with Docker or Kubernetes.
#
###############################################################################
#
# Script Variables:
...
if [ ! -d ${obmc_dir} ]; then
  for i in {1..3}; do
    ...
    if git clone https://github.com/openbmc/openbmc ${obmc_dir}; then
      break
    fi
    echo "Retrying git clone in 60 seconds..."
    sleep 60
  done
fi
