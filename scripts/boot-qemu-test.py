...
truncate_hash={'yosemite4' : default_trunc,}
...
# Docker networking
NET_FORWARDING=(f"hostfwd=:{IP}:{SSH_PORT}-:22,"
...
if MACHINE == "tacoma":
    ...
    # the active device.
    NIC =     ["-net", "nic,model=ftgmac100,netdev=netdev1", "-netdev",
                "user,id=netdev1"]
    NIC=NIC + ["-net", "nic,model=ftgmac100,netdev=netdev2", "-netdev",
                "user,id=netdev2"]
    NIC=NIC + ["-net", "nic,model=ftgmac100,netdev=netdev3", "-netdev",
                f"user,id=netdev3,{NET_FORWARDING}"]
    NIC=NIC + ["-net", "nic,model=ftgmac100,netdev=netdev4", "-netdev",
                "user,id=netdev4"]
else:
    NIC=      ["-net", "nic,model=ftgmac100,netdev=netdev1", "-netdev",
...
if MACHINE==DEFAULT_MACHINE:
    ...
    args=args+[
        ...
        "-netdev",   f"user,id=mynet,{NET_FORWARDING}"
        "-machine",  f"{DEFAULT_MACHINE}",
        "-m",        "256",
