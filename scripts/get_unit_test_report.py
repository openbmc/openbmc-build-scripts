...
for url in url_list:
    ...
    # Check Meson report.
    cmd = "find " + folder_name + " -name coveragereport"
    result = subprocess.check_output(cmd, shell=True)
    if result:
        ci_exists = "YES"
        result = result.splitlines()[0]
        cmd = "cp -rf " + result.strip() + " " + repo_report_dir
        subprocess.check_output(cmd, shell=True)

    # Check Auto make report.
    cmd = "find " + folder_name + " -name test-suite.log"
    result = subprocess.check_output(cmd, shell=True)
    if result:
        ci_exists = "YES"
        result = result.splitlines()[0]
        cmd = "cp " + result.strip() + " " + repo_report_dir
        subprocess.check_output(cmd, shell=True)
