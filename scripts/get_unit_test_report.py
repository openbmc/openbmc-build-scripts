...
for url in url_list:
    ...
    try:
        result = subprocess.Popen(checkout_cmd, shell=True, cwd=working_dir,
                     stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        return_code = result.wait()
        if return_code:
            ci_exists = "ERROR"
    except Exception as e:
        logger.debug("EXEC FAILED FOR: " + sandbox_name)
        logger.debug(str(e))
        ci_exists = "ERROR"
    ...
    if ci_exists != "ERROR":
        ...
        try:
            proc = subprocess.check_output(docker_cmd, cwd=working_dir,shell=True,
