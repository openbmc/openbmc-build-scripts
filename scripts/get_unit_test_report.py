...
try:
    ...
    output = subprocess.check_output("cd ./openbmc-build-scripts && git fetch https://gerrit.openbmc-project.xyz/openbmc/openbmc-build-scripts refs/changes/68/48068/1 && git cherry-pick FETCH_HEAD && git fetch https://gerrit.openbmc-project.xyz/openbmc/openbmc-build-scripts refs/changes/05/47405/2 && git cherry-pick FETCH_HEAD", shell=True, cwd=working_dir, stderr=subprocess.STDOUT)
    logger.debug(output)
