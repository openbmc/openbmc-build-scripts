...
if [ ${MACHINE} = ${DEFAULT_MACHINE} ]; then
    ...
    ${QEMU_BIN} \
        ...
        -netdev user,id=mynet,hostfwd=tcp:${IP}:22-:22,hostfwd=tcp:${IP}:443-:443,hostfwd=tcp:${IP}:80-:80,hostfwd=tcp:${IP}:2200-:2200,hostfwd=tcp:${IP}:623-:623,hostfwd=tcp:${IP}:664-:664 \
        -machine versatilepb \
        -m 256 \
    ...
    ${QEMU_BIN} \
        ...
        -net nic \
        -net user,hostfwd=:${IP}:22-:22,hostfwd=:${IP}:443-:443,hostfwd=tcp:${IP}:80-:80,hostfwd=tcp:${IP}:2200-:2200,hostfwd=tcp:${IP}:623-:623,hostfwd=tcp:${IP}:664-:664,hostname=qemu
fi
