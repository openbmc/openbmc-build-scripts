...
def process_issue(node, file_data):
    ...
    <issues>
        <issue name="infrastructure">
            ...
            <issue name="connection">
                <regex>.*failed to connect to simulation server</regex>
            </issue>
            <regex>\*\*ERROR\*\* Ending.*autoipl\.C with exit code of 1</regex>
            <regex>.*Issuing: vexec.*\nRunning on.*\n.*=*\n.*\*\*ERROR\*\*.*</regex>
            <regex>ERROR: failed to.*simulation.*rc=1</regex>
            <regex>ERROR: test timeout failure</regex>
            <regex>.*Exit code from private simulation.*is 1</regex>
            <regex>.*open_service_processor_session failure.*telnet.*retries</regex>
    ...
    (["issues", "connection"], "failed to connect to simulation server",
    "(.*failed to connect to simulation server)).
