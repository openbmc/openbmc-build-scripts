...
class Meson(BuildSystem):
    ...
    def _inject_conditions_into_html(coverage_dir, all_files_enriched):
        ...
        if not re.search(
            ...
            header_pat = (
                r"(?P<func>"
                r'<div role="rowheader"[^>]*>Function \(File:Line\)</div>)\s*'
                r'(?P<calls><div role="rowheader"[^>]*>Calls</div>)\s*'
                r'(?P<lines><div role="rowheader"[^>]*>Lines</div>)\s*'
                r"(?P<branches>"
                r'<div role="rowheader"(?P<br_attrs>[^>]*)>Branches</div>)\s*'
        ...
        def _open_tag(
            cell_html,
            default_class="color-fg-muted flex-auto min-width-0",
        ):
            m_open = re.match(
                r'\s*(<div role="gridcell"[^>]*>)', cell_html or ""
            )
            if m_open:
        ...
        def _row_inject(m):
            ...
            if entry is not None and line_found:
                line_cov_text = (
                    ...
                    f"<br/>Total Line: {line_found}"
                    f"<br/>Covered lines: {line_hit}"
                )
            elif entry is not None:
                line_cov_text = (
                    f"Line coverage: {lines_pct_text}"
                    f"<br/>Total Line: {line_found}"
                    f"<br/>Covered lines: {line_hit}"
                )
            else:
            ...
            if entry is not None and br_found:
                branch_cov_text = (
                    f"Branch Coverage Percentage:"
                    f" {Meson._pct(br_hit, br_found):.1f}%"
                    f"<br/>Total Branch: {br_found}"
                    f"<br/>Covered Branch: {br_hit}"
                )
            elif entry is not None:
                branch_cov_text = (
                    f"Branch Coverage Percentage: {branches_pct_text}"
                    f"<br/>Total Branch: {br_found}"
                    f"<br/>Covered Branch: {br_hit}"
                )
            else:
            ...
            if m_blk:
                ...
                blk_cov_text = (
                    ...
                    f"<br/>Total Block: {blk_found}"
                    f"<br/>Covered Block: {blk_hit}"
                )
            blocks_cell = (
                _open_tag(m.group("blocks")) + blk_cov_text + "</div>"
            )
    ...
    def _generate_lcov_function_json(
        ...
        def _flush_file_record(
            sf, fn_map, fn_counts, line_cov, line_br, line_cond
        ):
                ...
                all_files_enriched.append(
                    {
                        ...
                        "_lines_hit": sum(
                            1 for v in line_cov.values() if v > 0
                        ),
                        "_branches_found": sum(
        ...
        with open(info, "r") as f:
            for line in f:
                ...
                if s.startswith("SF:"):
                    ...
                    current_file = s[3:]
                    fn_map, fn_counts, line_cov, line_br, line_cond = (
                        _reset_state()
                    )
        ...
        try:
            ...
            try:
                self._inject_conditions_into_html(
                    coverage_dir, all_files_enriched
                )
            except Exception:
                print(
                    "Failed to inject Condition coverage "
                    "into index.functions.html"
                )
        except Exception:
    ...
    def _generate_gcovr_fallback_json(self, coverage_dir, json_root_dir):
        ...
        for file_data in report.get("files", []):
            ...
            for ln in file_data.get("lines", []):
                ...
                try:
                    ...
                    if isinstance(blks, list) and blks:
                        blk_found = len(blks)
                        blk_hit = sum(
                            1 for b in blks if (b.get("count", 0) or 0) > 0
                        )
            ...
            for fn in file_data.get("functions", []):
                name = fn.get("name")
                start = (
                    fn.get("start_line") or fn.get("lineno") or fn.get("line")
                )
                cnt = fn.get("execution_count") or fn.get("count") or 0
                if name is None or start is None:
                    continue
                fun_list.append(
                    {"name": name, "line": int(start), "count": int(cnt)}
                )
        ...
        try:
            ...
            print(
                "Failed to write aggregate index.functions.json"
                " (gcovr fallback)"
            )
