...
obmc_qemu_docker=`docker run -d -u root -e HOME=${HOME} -e \
  QEMU_RUN_TIMER=${QEMU_RUN_TIMER}  -w "${HOME}"           \
  -v "${UPSTREAM_WORKSPACE}":"${HOME}"                     \
  -t ${DOCKER_IMG_NAME} ${HOME}/boot-qemu-test.sh`
...
git clone https://github.com/openbmc/openbmc-test-automation.git ./obmc-test
cd ${ROBOT_CODE_HOME}
...
docker run -u root -w ${HOME} -v ${WORKSPACE}:${HOME} \
  -t ${DOCKER_IMG_NAME} ${HOME}/${DOCKER_ROBOT_SCRIPT}
