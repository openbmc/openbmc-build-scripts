...
# the robot CI test suite against it.(ROBOT CI TEST CURRENTLY WIP)
#
###############################################################################
#
# Parameters used by the script:
#  UPSTREAM_WORKSPACE = The directory from which the QEMU components are being
#                       imported from. Generally this is expected to be the
#                       build directory that is generated by the OpenBMC
#                       build-setup.sh script when run with "target=qemu".
...
#                       builds, moved the mounting location to be the same to
...
#                       should also be changed. Otherwise left the way it is.
#
###############################################################################
