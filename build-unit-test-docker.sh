...
for package in $PKGS
do
    DEPCACHE+=${package}:$(git ls-remote https://github.com/openbmc/${package} | grep 'refs/heads/master' | awk '{ print $1 }'),
done
...
RUN git clone https://github.com/amboar/phosphor-logging && \
cd phosphor-logging && \
./bootstrap.sh && \
