...
# Kubernetes Deployment into a kubernetes cluster.
################################################################################
# Launch Variables:
...
if [[ ${LAUNCH} == "docker" ]]; then
  ...
  echo "Setup the PVC's Needed"
  source ./kubernetes/init-jenkins
  echo "Launch the Master"
  source ./kubernetes/kubernetes-launch Jenkins-Master false false
