...
try:
    ...
    ret=docker_inspect_check("docker-inspect: obmc_qemu_docker={obmc_qemu_docker} no instance", container=obmc_qemu_docker, fail=False)
    if ret.returncode == 0:
        persist=True
    else:
        persist=False
...
if LAUNCH == "local":
    ...
    if persist == False:
        try:
            #    "--rm", # remove for persistent logging
...
#    trap '' PIPE
    ...
    if IPAddress!="":
        print(f"IPAddress={IPAddress} ok")
    else:
