...
def pexpect_child_send_expect(child):
    ...
    def mlist(token):
        ...
        return [token] + [pexpect.EOF, pexpect.TIMEOUT, "\wogin\stimed"]
    ...
    try:
        while True:
            if match_token(
                child.expect(mlist(".* login: "), timeout=1000), "login"
            ):
                continue
            ...
            if match_token(
                child.expect(mlist("Password:"), timeout=1000), "password"
            ):
                continue
            ...
            if match_token(
                child.expect(mlist(".*root@.*:~#.*"), timeout=1000), "prompt"
            ):
                continue
            else:
                break
