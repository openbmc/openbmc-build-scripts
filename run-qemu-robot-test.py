...
#                       "./tmp/deploy/images/"
#
#  OBMC_QEMU_DOCKER   = Use a persistent Docker container if runtime verified
#  KEEP_PERSISTENT    = (testing) if set, it will not attempt to stop the qemu
#                       container so that it is left persistent for testing.
#  INTERACT           = (testing) if set, it will allow for interactive
#                       session in the qemu container to bypass pexpedt issues.
#  HOSTBINPERSIST     = (testing) if set then the qemu container is left running
#                       and not stopped.
#
###############################################################################
...
def docker_inspect_check(container, exit_failed=False):
    ...
    if ret.returncode != 0:
        ...
        return ret  # both the exit code and .stdout output
...
if PROXY == "False":
    ...
    proxy_args = env_proxy_build_args(PROXY)

# rework this above
# proxy_args = env_proxy_build_args(os.enviro.get("PROXY", []))
...
KEEP_PERSISTENT = os.environ.get("KEEP_PERSISTENT", False)
print(f"KEEP_PERSISTENT = {KEEP_PERSISTENT}")
...
if OBMC_QEMU_DOCKER:
    ...
    if ret.returncode != 0:  # success on '0'
        OBMC_QEMU_DOCKER = False  # failed the check, clobber invalid value
...
def docker_pull_qemu_image():
    # check to see if DOCKER_IMG_NAME has arguments
    global DOCKER_IMG_NAME
    args = str.split(DOCKER_IMG_NAME)
    ...
    if len(args) > 1:
        if args[0] == "pull":
            ...
            DOCKER_IMG_NAME = args[1]
        else:
            print("docker_pull_qemu: not pull case")
            default = True
    else:
        default = True
        print("docker_pull_qemu: len isn't greater than 1")

    if default:
        exe = ([DIR + "/scripts/build-qemu-robot-docker.sh", DOCKER_IMG_NAME],)

    print(f"docker_pull_qemu_image: exe ={exe}")
...
def qemu_child_new_container(args):
    ...
    OBMC_QEMU_DOCKER = docker_run(execargs, "OBMC_QEMU_DOCKER")
    child, ret1 = pexpect_child_from_container(OBMC_QEMU_DOCKER)
    return child, None, execargs
...
def qemu_child_get_ip(container):
    ...
    ret = docker_inspect_check(container)
    if ret.returncode != 0:
    ...
    if LAUNCH == "local":
        ...
        if HOSTBIN:
            ...
            if OBMC_QEMU_DOCKER:
                ...
                child, file, execargs = qemu_child_new_container(
                    host_networking_arg(hostnetworking) + rm
                )
        ...
        if not (KEEP_PERSISTENT or HOSTBINPERSIST):
