...
# This is specific to the fby35 platform but can be made generic
def qemu_child_from_hostbin(MACHINE_QEMU):
...
def qemu_child_get_ip(container, ip_default):
    ...
    if LAUNCH == "local":
        ...
        if HOSTBIN:
            ...
            if not HOSTBINPERSIST:
                ...
                child, file = qemu_child_from_hostbin(MACHINE_QEMU)
                pexpect_child_send_expect(child)
            else:
                # use existing qemu otherwise
                print("Using existing qemu instances")
        else:
            if OBMC_QEMU_DOCKER:
            ...
            ip_address = qemu_child_get_ip(OBMC_QEMU_DOCKER, ip_address)
            child = None
