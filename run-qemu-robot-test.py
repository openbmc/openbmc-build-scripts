...
DOCKER_IMG_NAME = os.environ.get("DOCKER_IMG_NAME", "False")
if DOCKER_IMG_NAME != "False":
    dockerimg = True
else:
    dockerimg = False
    DOCKER_IMG_NAME = "openbmc/ubuntu-robot-qemu"
...
if os.environ.get("persistentqemu", "False") != "False":
    print("persistentqemu True")
    persistentqemu = True
else:
    print("persistentqemu False")
    persistentqemu = False
...
if os.environ.get("hostbin", "False") != "False":
    ...
    if OBMC_QEMU_DOCKER != "False":
        ...
        if ret.returncode == 0:  # success on '0'
            print("persistentdocker True")
            persistentdocker = True
        else:  # means it's invalid
            print("persistentdocker False")
            persistentdocker = False
    else:
        print("persistentdocker False")
        persistentdocker = False
...
def pexpect_child_from_hostbin():
    ...
    args += [
        ...
        "nic",
        "-net",
        f"user,hostfwd=::{SSH_PORT}-:443,hostfwd=::{HTTPS_PORT}-:22,hostname=qemu",
    ]
    args += ["-nographic"]
...
def qemu_child_new_container(netargs, rm):
    ...
    ret = pexpect_child_from_container(OBMC_QEMU_DOCKER)
    return ret + [f"{OBMC_QEMU_DOCKER}"]
...
def main():
    ...
    if LAUNCH == "local":
        ...
        if hostbin:
            hostnetworking = True
            if not persistentqemu:
                # spawn new qemu host networking
                child, file = pexpect_child_from_hostbin()
                pexpect_child_send_expect(child)
        else:
            if persistentdocker:
                print("exit to login prompt >")
                qemu_child_persist_interact(OBMC_QEMU_DOCKER)
            else:
                hostnetworking = True
                # new container instance
                child, file, OBMC_QEMU_DOCKER = qemu_child_new_container(
        ...
        args = host_networking_arg(hostnetworking) + [
            "--env",
            f"HOME={HOME}",
            "--env",
        ...
        if not (
            pexpect_child_close(child) or KEEP_PERSISTENT or persistentqemu
        ):
