...
def qemu_child_new_container(netargs, rm):
    ...
    return pexpect_child_from_container(OBMC_QEMU_DOCKER)
    ...
    if LAUNCH == "local":
        ...
        if hostbin:
            ...
            if persistentdocker:
                ...
                docker_inspect = True
                child, file = qemu_child_new_container(
                        netargs,
                        rm
                        ) # OBMC_QEMU_DOCKER

            if not persistentdocker:
        ...
        if not (pexpect_child_close(child) or KEEP_PERSISTENT or persistentqemu):
