...
def main():
    ...
    if LAUNCH == "local":
        ...
        # localhost networking hack here. Needs to be run inside a pod (podman) or use --network=host so that it can resolve
        # alias ssh_pass_openbmc="sshpass -p 0penBmc ssh root@127.0.0.1 -p 2222 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null systemctl is-system-running"

        # wait for ssh to come up. Won't completely guarantee that HTTPS/SSH ports are up but it's a start
        os.system(
            "sshpass -p 0penBmc ssh root@127.0.0.1 -p 2222 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null systemctl is-system-running && sleep 10"
        )
